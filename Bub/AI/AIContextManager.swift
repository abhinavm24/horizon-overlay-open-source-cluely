...
class AIContextManager: ObservableObject {
    ...
    func getFrontmostUserWindow() -> (pid: pid_t, windowID: CGWindowID)? {
        ...
        for window in windowList {
            if let layer = window[kCGWindowLayer as String] as? Int,
               layer == 0, // Layer 0 means "normal window" (not dock/menu bar/etc)
    ...
    func getAllSelectedTextFromOtherApps() async -> String? {
        if let window = getFrontmostUserWindow() {
/*
            if let text = findSelectedText(in: window.0), !text.isEmpty {
                return text
            }
  */
            return await NSPasteboard.getSelectedText(wid: window.windowID)
        }
